...
- **本地持久化**：导入通过校验后会自动保存到浏览器本地存储（刷新自动恢复，可点“恢复示例”清除）
- **自动判分（客观题/部分题型）**
  - 单选 / 判断：对比答案
...
项目内 TypeScript 类型定义位于：`src/types/exam.ts`。更完整的规则与“从原始试卷+答案生成 JSON”的指南见：`docs/EXAM_JSON_RULES.md`。
