...
export default function MarkdownContent({ content, enableMath = false }: Props) {
  return (
    <div className="space-y-2">
      <ReactMarkdown
        remarkPlugins={[remarkGfm, remarkBreaks, ...(enableMath ? [remarkMath] : [])]}
        rehypePlugins={enableMath ? [rehypeKatex] : []}
        components={{
          p: ({ children }) => <p className="m-0 break-words">{children}</p>,
          ul: ({ children }) => <ul className="m-0 list-disc pl-6 space-y-1">{children}</ul>,
          ol: ({ children }) => <ol className="m-0 list-decimal pl-6 space-y-1">{children}</ol>,
